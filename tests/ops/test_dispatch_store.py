...
class TestStoreCall:
    ...
    async def test_preserves_existing_enrichment(self):
        """store_call returns existing enriched doc without re-enriching or overwriting."""
        ...
        # Pre-store enriched doc
        async with DispatchStore() as store:
            await store.upsert(doc)

        # store_call with fresh call (same ID) — should return existing enriched doc
        async with DispatchStore() as store:
            result = await store.store_call(call)

            # Enrichment preserved
            assert result.analysis.incident_commander == "BN31"
            assert result.analysis.summary == "Patient fall"

            # _enrich was NOT called (existing enrichment short-circuited)
            store._enrich.assert_not_called()

            # Cosmos doc still has enrichment
            stored = await store.get("uuid-sc-3", "2026")
            assert stored.analysis.incident_commander == "BN31"
            assert stored.analysis.summary == "Patient fall"
...
# ------------------------------------------------------------------
# sync_recent (diff-then-store for background sync)
# ------------------------------------------------------------------


class TestSyncRecent:
    async def test_stores_only_new_completed(self):
        """Pre-stored calls are skipped; only new ones are stored."""
        existing = _make_call(id="uuid-sr-1", is_completed=True)
        new_call = _make_call(id="uuid-sr-2", long_term_call_id="26-001700", is_completed=True)

        # Pre-store one call
        async with DispatchStore() as store:
            await store.store_call(existing)

        async with DispatchStore() as store:
            with patch.object(DispatchStore, "_fetch_recent", return_value=[existing, new_call]):
                count = await store.sync_recent(days=2)

        assert count == 1

        # New call should be in store
        async with DispatchStore() as store:
            stored = await store.get("uuid-sr-2", "2026")
            assert stored is not None

    async def test_skips_open_calls(self):
        open_call = _make_call(id="uuid-sr-3", is_completed=False)

        async with DispatchStore() as store:
            with patch.object(DispatchStore, "_fetch_recent", return_value=[open_call]):
                count = await store.sync_recent(days=2)

        assert count == 0

    async def test_returns_zero_when_all_exist(self):
        call = _make_call(id="uuid-sr-4", is_completed=True)

        async with DispatchStore() as store:
            await store.store_call(call)

        async with DispatchStore() as store:
            with patch.object(DispatchStore, "_fetch_recent", return_value=[call]):
                count = await store.sync_recent(days=2)

        assert count == 0

    async def test_returns_zero_for_empty_fetch(self):
        async with DispatchStore() as store:
    ...
    async def test_passes_days_to_fetch(self):
        async with DispatchStore() as store:
            with patch.object(DispatchStore, "_fetch_recent", return_value=[]) as mock:
                await store.sync_recent(days=7)
            mock.assert_called_once_with(7)


# ------------------------------------------------------------------
# list_recent_with_open (pure-read merge)
# ------------------------------------------------------------------


class TestListRecentWithOpen:
    async def test_merges_stored_and_open(self):
        """Returns open calls + stored calls with no duplicates."""
        # Pre-store a completed call
        completed = _make_doc(id="uuid-lro-1", is_completed=True, nature="Fire Alarm")
        async with DispatchStore() as store:
            await store.upsert(completed)

        open_call = _make_call(id="uuid-lro-2", is_completed=False, nature="Medical Aid")

        async with DispatchStore() as store:
            with patch.object(
                DispatchStore,
                "_fetch_open",
                return_value=[open_call],
            ):
                docs = await store.list_recent_with_open()

        assert len(docs) == 2
        # Open calls first
        assert docs[0].id == "uuid-lro-2"
        assert docs[1].id == "uuid-lro-1"

    async def test_deduplicates_open_and_stored(self):
        """If a call appears in both stored and open, only open version is kept."""
        doc = _make_doc(id="uuid-lro-3", is_completed=True)
        async with DispatchStore() as store:
            await store.upsert(doc)

        # Same ID returned as open
        open_call = _make_call(id="uuid-lro-3", is_completed=False)
        ...
        assert len(docs) == 1
        assert docs[0].id == "uuid-lro-3"

    async def test_empty_store_and_no_open(self):
        async with DispatchStore() as store:
            with patch.object(DispatchStore, "_fetch_open", return_value=[]):
                docs = await store.list_recent_with_open()
        assert docs == []

    async def test_only_open_calls(self):
        open_call = _make_call(id="uuid-lro-4", is_completed=False)
    ...
    async def test_only_stored_calls(self):
        doc = _make_doc(id="uuid-lro-5", is_completed=True)
        async with DispatchStore() as store:
            await store.upsert(doc)
