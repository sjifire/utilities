#!/bin/bash
# =============================================================================
# SJI Fire Ops Server — Azure Infrastructure Setup
# =============================================================================
#
# One-time script to provision Azure resources for the remote ops server.
...
#   Our ops server uses the NoSQL/SQL API (azure.cosmos.aio → *.documents.azure.com).
...
if should_run 1; then
    ...
    EXISTING_APP=$(az ad app list --display-name "SJI Fire Ops Server" --query "[0].appId" -o tsv 2>/dev/null || true)

    if [ -n "$EXISTING_APP" ]; then
        MCP_CLIENT_ID="$EXISTING_APP"
        warn "App registration already exists: $MCP_CLIENT_ID"
    else
        info "Creating app registration 'SJI Fire Ops Server'..."
        MCP_CLIENT_ID=$(az ad app create \
            --display-name "SJI Fire Ops Server" \
    ...
    az ad app update --id "$MCP_CLIENT_ID" \
        --web-redirect-uris \
            "https://ops.${DOMAIN}/callback" \
            "https://ops.${DOMAIN}/.auth/login/aad/callback" \
            "https://mcp.${DOMAIN}/callback" \
            "https://mcp.${DOMAIN}/.auth/login/aad/callback"
    ok "Web redirect URIs: ops.* + mcp.* (old domain kept for redirect)"
...
# =============================================================================
# Phase 6: Custom Domain (ops.sjifire.org)
# =============================================================================
...
if should_run 6; then
    ...
    if [ -n "$DNS_RG" ]; then
        ...
        # Create TXT verification record (asuid.ops → verification ID)
        info "Creating TXT verification record: asuid.ops.$DOMAIN"
        az network dns record-set txt add-record \
            --resource-group "$DNS_RG" \
            --zone-name "$DOMAIN" \
            --record-set-name "asuid.ops" \
        ...
        echo "    asuid.ops.$DOMAIN    TXT    $VERIFICATION_ID"
        echo ""
    ...
    # --- Keep mcp.sjifire.org as secondary domain for redirects ---
    # The server's _OldDomainRedirectMiddleware 301-redirects mcp.* → ops.*
    # so old bookmarks, Claude.ai configs, and shared links keep working.
    OLD_DOMAIN="mcp.${DOMAIN}"

    if [ -n "$DNS_RG" ]; then
        info "Ensuring old domain $OLD_DOMAIN DNS records exist (for redirect)..."
        az network dns record-set cname set-record \
            --resource-group "$DNS_RG" \
            --zone-name "$DOMAIN" \
            --record-set-name "mcp" \
        ...
        az network dns record-set txt add-record \
            ...
            --value "$VERIFICATION_ID" \
            --output none 2>/dev/null || warn "TXT record may already exist"
        ok "Old domain DNS records in place"
    fi

    info "Adding old hostname $OLD_DOMAIN to Container App (for redirect)..."
    az containerapp hostname add \
        --name "$CA_APP" \
        --resource-group "$RESOURCE_GROUP" \
        --hostname "$OLD_DOMAIN" \
    ...
    az containerapp hostname bind \
        ...
        --environment "$CA_ENV" \
        --validation-method CNAME \
        --output none 2>/dev/null || warn "Certificate binding may already exist"
    ok "Old domain $OLD_DOMAIN kept as secondary (301 → $CUSTOM_DOMAIN)"
...
# =============================================================================
#
# The ops server uses an LLM to extract structured data (IC, summary, outcome)
...
# uncomment the Azure OpenAI section below and run: ./scripts/setup-azure-ops.sh --phase 7
