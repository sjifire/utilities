"""CLI script to sync mail-enabled security groups from Aladtec data.

This uses the Exchange Online REST API (InvokeCommand endpoint) to manage
mail-enabled security groups, which don't create SharePoint sites like M365 groups.

Prerequisites:
1. Azure AD App Registration with Exchange.ManageAsApp permission
2. App assigned "Exchange Recipient Administrator" role in Exchange Admin Center
"""
...
STRATEGIES = {
    ...
    "allpersonnel": MailAllPersonnelGroupStrategy,
}


def print_result(result: MailFullSyncResult, dry_run: bool = False) -> None:
...
async def run_sync(
    ...
    """Run mail group sync for specified strategies.
    ...
    """
    logger.info("=" * 50)
    logger.info("Mail-Enabled Security Group Sync")
    logger.info("=" * 50)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sync mail-enabled security groups from Aladtec data",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.all:
        ...
        # Default to allpersonnel only
        strategies = ["allpersonnel"]
