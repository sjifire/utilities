...
**Signature sync** (`signature-sync`): Sets per-user custom attributes (CA6-8) on mailboxes and creates a transport rule that appends a personalized signature + org footer to all outgoing emails. Works with all email clients because signatures are applied server-side. Every outgoing message gets a signature (no per-sender deduplication — transport rules don't support dynamic per-sender conditions). See `scripts/signature_sync.py` and `core/extension_attrs.py` for slot assignments.
